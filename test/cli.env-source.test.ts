...
describe("loadCliEnvSource", () => {
  ...
  beforeEach(async () => {
    ...
    originalEnvValue = process.env.EZ_BOX_TEST_ENV_SOURCE;
    delete process.env.EZ_BOX_TEST_ENV_SOURCE;
  });
  ...
  afterEach(async () => {
    if (originalEnvValue === undefined) {
      ...
      process.env.EZ_BOX_TEST_ENV_SOURCE = originalEnvValue;
    }
  ...
  it("loads values from .env when process env is missing", async () => {
    ...
    await writeFile(envPath, "EZ_BOX_TEST_ENV_SOURCE=from-file\n");

    const result = await loadCliEnvSource(envPath);

    expect(result.EZ_BOX_TEST_ENV_SOURCE).toBe("from-file");
  });

  it("prefers process env values over .env", async () => {
    const envPath = join(tempDir, ".env");
    await writeFile(envPath, "EZ_BOX_TEST_ENV_SOURCE=from-file\n");
    process.env.EZ_BOX_TEST_ENV_SOURCE = "from-process";

    const result = await loadCliEnvSource(envPath);

    expect(result.EZ_BOX_TEST_ENV_SOURCE).toBe("from-process");
  });
