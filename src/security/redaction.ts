...
export function redactSensitiveText(value: string): string {
  return value
    ...
    .replace(SENSITIVE_ASSIGNMENT_REGEX, (match, delimiter: string) => {
      const delimiterIndex = match.indexOf(delimiter);
      return `${match.slice(0, delimiterIndex)}${delimiter}[REDACTED]`;
    })
